...
def match_against_decomposer(surface, lemma, expected_suffixes, force=False,
    ...
    # Decomposer root is a deeper derivation of the treebank lemma:
    # strip leading expected suffixes until the decomposer chain matches.
    for root, start_pos, chain, final_pos in candidates:
        if root == lemma_lower:
            continue
        chain_names = get_chain_names(chain)
    ...
    # Lemma already carries a possessive? try dropping the leading P*.
    if expected_filtered and expected_filtered[0] in POSS_MAP.values() and POSS_MAP.values():
