...
2. Create a deterministic train/validation split from those same gold entries/sequences using `config.validation_split` and `config.validation_seed`.
...
The `test` CLI command evaluates the current model on `data/trmor2018_treebank/trmor2018_gold_adapted.jsonl`.

That file is generated by `data/trmor2018_treebank/treebank_adapter.py` from `trmor2018_gold.conllu`. It is separate from validation; validation remains the deterministic seed-42 split used during training.
